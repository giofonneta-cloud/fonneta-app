...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-lg shadow-lg p-3 text-sm">
      ...
      <p className="text-slate-600">Ingresos: {fmt(data.value)}</p>
      <p className="text-slate-500">{data.payload.percentage.toFixed(1)}% del total</p>
    </div>
  );
};

export function TopClientesChart({ sales }: Props) {
  const data = useMemo(() => {
    const clientes: Record<string, number> = {};

    sales.forEach(s => {
      const nombre = s.cliente?.business_name || 'Sin cliente';
      clientes[nombre] = (clientes[nombre] || 0) + (Number(s.valor_venta_neto) || 0);
    });

    const total = Object.values(clientes).reduce((a, b) => a + b, 0);
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-slate-800">Top 5 Clientes</h3>
        <p className="text-xs text-slate-400 mt-1">Por ingresos totales</p>
            ...
            {data.map((cli, idx) => (
              <div key={idx} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-600 truncate" title={cli.nombreCompleto}>
                    {cli.nombre}
                  </span>
                </div>
                <span className="font-black text-slate-900">{cli.percentage.toFixed(1)}%</span>
              </div>
            ))}
          </div>
        </div>
