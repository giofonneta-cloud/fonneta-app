...
export function QuickActionsCard() {
    const actions = [
        ...
        },
        {
            ...
            href: '/dashboard/providers/invite',
            icon: (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {
            label: 'Configuración de Finanzas',
            ...
            <div className="flex flex-wrap gap-2">
                {actions.map((action) => (
                    <Link
                        key={action.href}
                        href={action.href}
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-colors ${colorClasses[action.color]}`}
                    >
                        {action.icon}
