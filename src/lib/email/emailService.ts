...
Cel: 318 254 4377`;
  ...
  /**
   * Enviar documento Release al proveedor/autor para su firma
   */
  async sendReleaseDocument(
    recipientEmail: string,
    recipientName: string,
    releaseNumber: string,
    tipoReleaseLabel: string,
    documentUrl?: string,
    attachments?: EmailAttachment[],
    ccEmail?: string
  ): Promise<void> {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1f2937;">
        <h2 style="color: #1d4ed8;">Release ${releaseNumber}</h2>
        <p>Estimado(a) <strong>${recipientName}</strong>,</p>
        <p>Le informamos que Fonneta Comunicaciones S.A.S. requiere de su firma en el siguiente documento de release:</p>
        ...
        <p style="margin-top: 16px; font-size: 14px; color: #374151;">
          Encontrará el documento adjunto a este correo en formato PDF. Por favor fírmelo y devuélvalo respondiendo a este correo o a través del canal indicado por su contacto en Fonneta.
        </p>
...
Fonneta Comunicaciones S.A.S. requiere de su firma en el documento de release ${releaseNumber} (${tipoReleaseLabel}).

Encontrará el documento adjunto a este correo en formato PDF. Por favor fírmelo y devuélvalo respondiendo a este correo.
${documentUrl ? `También puede verlo en Google Drive: ${documentUrl}\n` : ''}
Cordialmente,
Fonneta Comunicaciones S.A.S.
NIT 901.362.051-7
Carrera 6 #123A-74, Bogotá D.C.
Cel: 318 254 4377`;
    ...
    await this.sendEmail({
      to: recipientEmail,
      ...(ccEmail ? { cc: ccEmail } : {}),
      subject: `Release ${releaseNumber} — Requiere su firma - Fonneta Comunicaciones`,
      html,
      text: textAlternative,
      attachments,
    });
